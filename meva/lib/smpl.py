...
JOINT_REGRESSOR_TRAIN_EXTRA = osp.join(MEVA_DATA_DIR, 'J_regressor_extra.npy')
SMPL_MEAN_PARAMS = osp.join(MEVA_DATA_DIR, 'smpl_mean_params.npz')
...
def normalize_smpl_pose(pose_aa, tran = None, random_root = False):
    root_aa = pose_aa[0,:3]
    ...
    if not tran is None:
        tran[:,[0,1]] -= tran[0, [0,1]]
        tran[:,[2]] = tran[:,[2]] - tran[0, [2]] + 0.91437225
        tran = np.matmul(apply_mat, tran.T).T
    return pose_aa, tran
